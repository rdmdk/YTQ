...
if (!localStorage.news && !sessionStorage.news) {
    if (localStorage.id) {
        main.insertAdjacentHTML("beforeend", "<h2 id='news' class='h2 new wait' title='Check out the lite version of YTL!' data-color><span class='span'>Check out YTL!</span></h2>");
        document.body.addEventListener("click", (e) => {
            if (e.target.id === "news") {
...
saved.querySelector(".sync").addEventListener("click", () => {
    ...
    if (a.classList.contains("off")) {
        a.classList = "li sync on";
        localStorage.synced = "on";
    } else {
        a.classList = "li sync off";
