...
/// Sort
var s1 = Sortable.create(favorites, {
	group: '.channels',
	handle: '.drag',
	disabled: true
});
var s2 = Sortable.create(all, {
	group: '.channels',
	handle: '.drag',
	disabled: true
});
	...
	fetch(url)
		...
		.then((b) => {
			...
			b.items.forEach(c => {
				...
				latest.querySelector('.videos').insertAdjacentHTML('beforeend', '<li class="li" title="' + c.snippet.title.replace(/\'/gm, '&#39;') + ' - ' + new Date(c.snippet.publishedAt).getDate() + ' ' + new Date(c.snippet.publishedAt).toLocaleString('en-us', {
					month: 'short'
				}) + ' ' + new Date(c.snippet.publishedAt).getFullYear() + '" data-background><span class="span listing" data-id="' + vid + '" data-date="' + new Date(c.snippet.publishedAt).getDate() + ' ' + new Date(c.snippet.publishedAt).toLocaleString('en-us', {
					month: 'short'
...
function importurls(a) {
			...
			if (y.startsWith('PL')) {
				...
				fetch(url)
					...
					.then((b) => {
						...
						b.items.forEach(c => {
							if (!saved.querySelector('.videos').innerHTML.includes(c.snippet.resourceId.videoId)) list.push(c);
						});
						let s = setInterval(() => {
							if (x) {
								fetch(nexturl + x)
										...
										setTimeout(function() {
											if (!saved.querySelector('.videos').innerHTML.includes(a.snippet.resourceId.videoId)) {
												...
												fetch(url)
													...
													.then((b) => {
														b.items.forEach(c => {
															if (!saved.querySelector('.videos').innerHTML.includes(c.id)) saved.querySelector('.videos').insertAdjacentHTML('beforeend', '<li class="li" title="' + c.snippet.title.replace(/\'/gm, '&#39;') + ' - ' + new Date(c.snippet.publishedAt).getDate() + ' ' + new Date(c.snippet.publishedAt).toLocaleString('en-us', {
																month: 'short'
															}) + ' ' + new Date(c.snippet.publishedAt).getFullYear() + '" data-background><span class="span listing" data-id="' + c.id + '" data-date="' + new Date(c.snippet.publishedAt).getDate() + ' ' + new Date(c.snippet.publishedAt).toLocaleString('en-us', {
																month: 'short'
															}) + ' ' + new Date(c.snippet.publishedAt).getFullYear() + '" data-channel="' + c.snippet.channelTitle.replace(/\'/gm, '&#39;') + '" data-channel-id="' + c.snippet.channelId + '"><span class="span image"><img class="img" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" style="background-image:url(' + c.snippet.thumbnails.medium.url + '"></span><span class="span title">' + c.snippet.title.replace(/\'/gm, '&#39;') + '</span></span><span class="save" title="Save" data-color><i class="fa fa-plus"></i></span></li>');
														});
													})
	...
	setTimeout(() => {
		localStorage.saved = saved.querySelector('.videos').innerHTML;
	}, 4e3);
}
